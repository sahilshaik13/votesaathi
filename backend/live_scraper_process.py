...
if __name__ == "__main__":
    logger.info("Realtime Election Scraper started.")
    ACTIVE_THRESHOLD = 600 # 10 minutes (If no users for 10 min, stop scraping)
    ...
    while True:
        try:
            ...
            if time_since_active > ACTIVE_THRESHOLD:
                logger.info(f"No active users detected (last active {int(time_since_active)}s ago). Skipping cycle to save costs.")
